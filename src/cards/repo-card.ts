...
export function renderRepoCard(
  ...
  const {
    ...
    header = show_owner ? nameWithOwner : name,
    langName = (primaryLanguage && primaryLanguage.name) || "Unspecified",
    langColor = (primaryLanguage && primaryLanguage.color) || "#333",
    shiftText = langName.length > 15 ? 0 : 30;
  ...
  const multiLineDescription = wrapTextMultiline(desc),
    ...
    lineHeight = 10,
    height = (descriptionLines > 1 ? 120 : 110) + descriptionLines * lineHeight,
    i18n = new I18n({
      locale,
    ...
    totalStars = kFormatter(stargazers.totalCount),
    totalForks = kFormatter(forkCount);

  function getBadgeSVG(label: string) {
    return `
  ...
  function iconWithLabel(icon: string, label: string, testid: string) {
    return `
