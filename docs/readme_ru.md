...
Вы можете увидеть, как выглядит любая тема [здесь](../themes/README.md) или в [конфигурационном файле тем](../themes/index.js) и **вы также можете помочь в создании новых тем**, если захотите :D.

### Персонализация

Вы можете изменить вид вашей `карточки статистики` и `карточки репозитория`, добавив соответствующие параметры к URL запросу.
...
- `border_color` - цвет рамки карточки _(задаётся шестнадцатеричным числом)_ (Ни на что не влияет, если рамка отключена, т.е. если установлен параметр `hide_border`)
...
> Примечание насчёт кэширования: карточка репозитория по умолчанию кэшируется на 4 часа (или 14400 секунд), если количество форков репозитория меньше 1000. В противном случае, карточка кэшируется на 2 часа (или 7200 секунд). Учтите, что кэш может храниться не менее 2 часов и не более суток.
...
- `hide` - скрывает указанные языки программирования из карточки _(перечисляются через запятую)_ 
...
- `langs_count` - устанавливает количество языков, отображаемых в карточке _(значение от 1 до 10)_. По умолчанию отображается 5 языков.
...
Чтобы изменить число отображаемых языков в карточке, установите нужное значение в параметр `&langs_count=`. Допустимые значения этого параметра - целые числа от 1 до 10. По умолчанию в карточке отображается 5 языков.
