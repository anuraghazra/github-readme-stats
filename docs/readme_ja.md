...
内蔵されているテーマを使用すれば、[手動のカスタマイズ](#テーマを自分でカスタマイズする)を行うことなくカードの外観を変更することができます。
...
> :warning: **重要:**
> [パーセントエンコーディング](https://ja.m.wikipedia.org/wiki/パーセントエンコーディング) で指定されているように、プログラミング言語の名前は URL エンコードされている必要があります。
