...
You can customize the width and height of the stats card using the `card_width` and `card_height` parameters to better fit your layout:

```md
![Anurag's GitHub stats](https://github-readme-stats.vercel.app/api?username=anuraghazra&card_width=600&card_height=300)
```
...
| `card_height` | Sets the card's height manually. | number | `195px  (approx.)` |
...
> [!NOTE]\
> When hide\_rank=`true`, the minimum card width is 270 px + the title length and padding. When hide\_rank=`false`, the minimum card height is 150 px to accommodate the rank circle.
