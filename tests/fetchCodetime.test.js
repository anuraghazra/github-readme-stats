...
const { fetchWakatimeStats, fetchCodetimeStats } = require("../src/fetchers/codetime-fetcher");
...
describe("Codetime fetcher", () => {
    it("should fetch correct codetime data", async () => {
        const username = "Leonel";
        mock
            .onGet(
                `https://app.software.com/api/v1/users/${username}/stats/?is_including_today=true`,
            )
            .reply(200, codeTimeData);

        const repo = await fetchCodetimeStats({ username });
    ...
    it("should throw error", async () => {
        mock.onGet(/\/https:\/\/app.software\.com\/api/).reply(404, codeTimeData);

        await expect(fetchCodetimeStats("noone")).rejects.toThrow(
            "CodeTime user not found, make sure you have a codetime profile",
        );
    });
});

module.exports = { codeTimeData };
